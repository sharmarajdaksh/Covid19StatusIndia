...
export class CountryDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._service.getStateDailyData().subscribe(res=>{
      ...
      let index1:number=0;
      let value:number=0;
      while(index1 < this.daily_changes.length){
        for(let i=0;i<this.states_codes.length;i++){
          value = value  + (Number(this.daily_changes[index1][this.states_codes[i]]))
        }
        this.lineChartDataset[0].data.push(value);
        this.dates_List.push(this.daily_changes[index1]['date'])  
        value =0;
        index1 = index1+6;
      }
